...
contract MamaBondStakeDepository is Ownable {
    ...
    address public immutable MAMA; // intermediate reward token from treasury
    address public immutable sMAMA; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints MAMA when receives principle
    ...
    struct Bond {
        uint gonsPayout; // sMAMA gons remaining to be paid
        uint mamaPayout; // mama amount at the moment of bond
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 MAMA ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) MAMA
         */
        ...
        IERC20( MAMA ).approve( staking, payout );
        IStaking( staking ).stake( payout, address(this) );
        
        uint stakeGons=ISMAMA(sMAMA).gonsForBalance(payout);

        // depositor info is stored
        _bondInfo[ _depositor ] = Bond({ 
            gonsPayout: _bondInfo[ _depositor ].gonsPayout.add( stakeGons ),
            mamaPayout: _bondInfo[ _depositor ].mamaPayout.add( payout ),
    ...
    function redeem( address _recipient, bool _stake) external returns ( uint ) {        
        ...
        uint _amount = ISMAMA(sMAMA).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sMAMA ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    /**
     *  @notice return bond info with latest sMAMA balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
     *  @return vesting uint
    ...
    function bondInfo(address _depositor) public view returns ( uint payout,uint vesting,uint lastTime,uint pricePaid ) {
        ...
        payout=ISMAMA(sMAMA).balanceForGons(info.gonsPayout);
    ...
    /**
     *  @notice calculate current ratio of debt to MAMA supply
    ...
    function percentVestedFor( address _depositor ) public view returns ( uint percentVested_ ) {
        ...
        uint blocksSinceLast =  uint32(block.timestamp).sub( bond.lastTime );
    ...
    /**
     *  @notice calculate amount of MAMA available for claim by depositor
     *  @param _depositor address
     *  @return pendingPayout_ uint
     */
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        uint percentVested = percentVestedFor( _depositor );
        uint payout = ISMAMA(sMAMA).balanceForGons(_bondInfo[ _depositor ].gonsPayout);
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or MAMA) to the DAO
     *  @return bool
     */
    function recoverLostToken( address _token ) external returns ( bool ) {
        require( _token != MAMA );
        require( _token != sMAMA );
